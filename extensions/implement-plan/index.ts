...
function sortByPrefix(tasks: TwTask[], getKey: (t: TwTask) => number | null): TwTask[] {
...
function buildExecutionPlan(jiraId: string, allTasks: TwTask[]): ExecutionPlan {
  ...
  const sortedPhases = sortByPrefix(phaseTasks, (t) => parsePhaseNumber(t.description));
  ...
  const currentSubtask = currentPhase ? firstNonDone(currentPhase.subtasks) : null;

  return { jiraId, phases, currentPhase, currentSubtask, totalSubtasks, doneSubtasks };
}

function planSummary(plan: ExecutionPlan): string {
  const lines: string[] = [`Execution plan for ${plan.jiraId}:`];
  lines.push(`Progress: ${plan.doneSubtasks}/${plan.totalSubtasks} subtasks done`);
  lines.push("");

  for (const phase of plan.phases) {
    const doneSubs = phase.subtasks.filter((s) => s.work_state === "done").length;
    const icon = phase.work_state === "done" ? "✓" : phase.work_state === "inprogress" ? "▶" : "○";
    lines.push(`  ${icon} Phase ${phase.number}: ${phase.name} [${phase.work_state}] (${doneSubs}/${phase.subtasks.length})`);
    for (const sub of phase.subtasks) {
      const sicon = sub.work_state === "done" ? "  ✓" : sub.work_state === "inprogress" ? "  ▶" : "  ○";
  ...
  if (plan.currentPhase) {
    ...
    if (plan.currentSubtask) {
      lines.push(`▶ Resume at: Phase ${plan.currentPhase.number} — subtask ${plan.currentSubtask.number} ${plan.currentSubtask.name}`);
    } else {
      lines.push(`▶ Resume at: Phase ${plan.currentPhase.number} (all subtasks done, phase not closed)`);
    }
  } else {
    lines.push("");
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      if (all.length === 0) {
        return {
          content: [{ type: "text", text: `No impl tasks found for ${params.jira_id}. Run bugwarrior-pull or check the Jira ID.` }],
          details: { found: false },
        };
      }
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      description: Type.Optional(
        Type.String({ description: "Task description (for confirmation output only)" }),
      ),
    }),
    async execute(_id, params) {
      await pi.exec("task", [params.uuid, "modify", `work_state:${params.state}`, "rc.confirmation:no"], {});
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      await pi.exec("task", [params.phase_uuid, "modify", "work_state:done", "rc.confirmation:no"], {});
      await pi.exec("task", [params.phase_uuid, "done", "rc.confirmation:no"], {});
  ...
  pi.registerCommand("implement", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!jiraId) {
        ctx.ui.notify("Usage: /implement <JIRA_ID>  e.g. /implement DP-121", "warning");
        return;
      }
