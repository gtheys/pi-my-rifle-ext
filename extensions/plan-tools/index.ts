...
async function resolveSpecPath(pi: ExtensionAPI, jiraId: string, summary: string): Promise<string> {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "tw_get_ticket",
    label: "TW: Get Ticket",
    description: "Fetch Jira ticket details from taskwarrior by Jira ID. Returns parsed task fields: description, jiradescription, jirasummary, jirastatus, jiraurl, jiraissuetype, jiraparent, tags, project.",
    promptSnippet: "Fetch Jira ticket details from taskwarrior",
    parameters: Type.Object({
      jira_id: Type.String({ description: "Jira ticket ID, e.g. IMP-7070 or DP-92" }),
    }),
    async execute(_id, params) {
      const tasks = await twExport(pi, [`jiraid:${params.jira_id}`, "+jira"]);
      if (tasks.length === 0) {
        return {
          content: [{ type: "text", text: `No taskwarrior task found for "${params.jira_id}". Run \`bugwarrior pull\` to sync, or provide ticket details manually.` }],
          details: { found: false },
        };
      }
      return {
  ...
  pi.registerTool({
    ...
    description: "Fetch the spec task for a Jira ticket. Returns the task and extracts the spec file path from its annotation (pattern: Spec(repo=<repo>): <path>). specPath is null if no annotation found.",
    ...
    async execute(_id, params) {
      ...
      if (tasks.length === 0) {
        return {
          content: [{ type: "text", text: `No spec task found for ${params.jira_id}.` }],
          details: { found: false, specPath: null },
        };
      }
      const task = tasks[0];
      const specPath = extractSpecPath(task);
      return {
        content: [{ type: "text", text: JSON.stringify({ task, specPath }, null, 2) }],
  ...
  pi.registerTool({
    name: "tw_get_phases",
    label: "TW: Get Phases",
    description: "Fetch all phase tasks (+phase tag) for a Jira ticket from taskwarrior.",
  ...
  pi.registerTool({
    ...
    description: "Fetch all implementation tasks (+impl tag) for a Jira ticket from taskwarrior.",
  ...
  pi.registerTool({
    ...
    description: "Compute the canonical spec file path for a Jira ticket. Resolves repo name from git remote, applies $LLM_NOTES_ROOT if set, generates a slug from the summary (max 5 words, lowercase, dashes). Returns the full absolute path.",
    ...
    parameters: Type.Object({
      ...
      summary: Type.String({ description: "Jira summary/title used to generate the slug" }),
    }),
    async execute(_id, params) {
  ...
  pi.registerTool({
    ...
    description: "Create a spec task in taskwarrior and annotate it with the spec file path. Sets work_state:approved and +spec tag. Project is prefixed with SalaryHero automatically. Returns the task UUID.",
    promptSnippet: "Create a spec task in taskwarrior with spec file annotation",
    parameters: Type.Object({
      jira_id: Type.String({ description: "Jira ticket ID" }),
      summary: Type.String({ description: "Jira summary/title" }),
      project: Type.String({ description: "Project suffix, e.g. 'backend'. Will be stored as SalaryHero.<project>." }),
      repo: Type.String({ description: "Repository name for the spec annotation" }),
      spec_path: Type.String({ description: "Relative spec file path, e.g. notes/specs/IMP-7070__slug.md" }),
    }),
    async execute(_id, params) {
      const addResult = await pi.exec("task", [
        "add",
      ...
      if (!match) throw new Error(`Failed to create spec task: ${addResult.stdout} ${addResult.stderr}`);

      const uuid = await getTaskUuid(pi, match[1]!);
      await pi.exec("task", [uuid, "annotate", `Spec(repo=${params.repo}): ${params.spec_path}`, "rc.confirmation:no"], {});

      return {
        content: [{ type: "text", text: `Created spec task (UUID: ${uuid})\nAnnotated: Spec(repo=${params.repo}): ${params.spec_path}` }],
        details: { uuid },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Create a phase task in taskwarrior. Sets work_state:todo and +impl +phase tags. Returns the task UUID — pass this as depends_uuid to tw_create_impl_task.",
    promptSnippet: "Create a phase task in taskwarrior, returns UUID for depends",
    parameters: Type.Object({
      jira_id: Type.String({ description: "Jira ticket ID" }),
      phase_number: Type.Number({ description: "Phase number, e.g. 1, 2, 3" }),
      phase_name: Type.String({ description: "Phase name, e.g. 'Database Schema'" }),
      project: Type.String({ description: "Project suffix. Will be stored as SalaryHero.<project>." }),
    ...
    async execute(_id, params) {
      ...
      const match = addResult.stdout.match(/Created task (\d+)/);
      if (!match) throw new Error(`Failed to create phase task: ${addResult.stdout} ${addResult.stderr}`);

      const uuid = await getTaskUuid(pi, match[1]!);

      return {
        content: [{ type: "text", text: `Created phase "${params.phase_number}. Phase: ${params.phase_name}" (UUID: ${uuid})` }],
        details: { uuid },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Create an implementation task in taskwarrior under a phase. Sets work_state:todo and +impl tag. Use the UUID from tw_create_phase as depends_uuid.",
    promptSnippet: "Create an implementation task under a phase in taskwarrior",
    parameters: Type.Object({
      jira_id: Type.String({ description: "Jira ticket ID" }),
      title: Type.String({ description: "Task title, e.g. '1.1 Add migration for users table'" }),
      project: Type.String({ description: "Project suffix. Will be stored as SalaryHero.<project>." }),
      repo: Type.String({ description: "Repository name" }),
      depends_uuid: Type.String({ description: "UUID of the parent phase task from tw_create_phase" }),
    }),
    async execute(_id, params) {
      const addResult = await pi.exec("task", [
        "add",
      ...
      const match = addResult.stdout.match(/Created task (\d+)/);
      if (!match) throw new Error(`Failed to create impl task: ${addResult.stdout} ${addResult.stderr}`);

      const uuid = await getTaskUuid(pi, match[1]!);

      return {
        content: [{ type: "text", text: `Created impl task "${params.title}" (UUID: ${uuid})` }],
        details: { uuid },
      };
    },
  });
  ...
  pi.registerCommand("plan", {
    description: "Create or iterate on an implementation plan for a Jira ticket",
