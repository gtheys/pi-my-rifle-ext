...
4. **Open the updated spec for review** — after saving, call the `open_in_pane` tool with the spec path (nvim opens it in a herdr `spec-review` pane). Skippable on user request; if herdr is unavailable the tool returns the path with a manual-open note — continue either way.
